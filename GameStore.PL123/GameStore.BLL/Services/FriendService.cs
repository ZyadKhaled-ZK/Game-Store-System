...
public class FriendService : IFriendService
{
    ...
    public async Task<List<User>> GetSuggestionsAsync(string userId, int count = 6)
    {
        var friendIds = await GetFriendIdsAsync(userId);
        var excludeIds = new List<string> { userId };
        excludeIds.AddRange(friendIds);

        var existingRequestIds = await _uow.Repository<Friendship>().Query()
            .Where(f => f.RequesterId == userId || f.ReceiverId == userId)
            .Select(f => f.RequesterId == userId ? f.ReceiverId : f.RequesterId)
            .Distinct()
            .ToListAsync();
        excludeIds.AddRange(existingRequestIds);
