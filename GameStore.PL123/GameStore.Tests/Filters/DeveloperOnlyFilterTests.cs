...
public class DeveloperOnlyFilterTests
{
    private readonly Mock<IUserService> _userServiceMock;

    public DeveloperOnlyFilterTests()
    {
    ...
    [Fact]
    public async Task DeveloperUser_ThroughCookie_Authorized()
    {
        var context = CreateContext(isAuthenticated: true, roleClaim: "DEVELOPER");
        var filter = new DeveloperOnlyFilter(_userServiceMock.Object);
    ...
    [Fact]
    public async Task CustomerUser_ThroughCookie_RedirectsToBecomeDeveloper()
    {
        ...
        redirect.ControllerName.Should().Be("BecomeDeveloper");
    }

    [Fact]
    public async Task AdminUser_ThroughCookie_RedirectsToBecomeDeveloper()
    {
    ...
    [Fact]
    public async Task UnauthenticatedUser_WithDeveloperSession_Authorized()
    {
        var context = CreateContext(sessionId: "u1", sessionRole: "DEVELOPER");
        _userServiceMock.Setup(s => s.GetByIdAsync("u1"))
            .ReturnsAsync(new User { Id = "u1", Role = Role.DEVELOPER });
        var filter = new DeveloperOnlyFilter(_userServiceMock.Object);
    ...
    [Fact]
    public async Task SessionRoleMismatch_UpdatesSessionFromDB()
    {
        ...
        context.Result.Should().BeNull();
        context.HttpContext.Session.GetString("Role").Should().Be("DEVELOPER");
    }

    [Fact]
    public async Task SessionRoleMismatch_NonDeveloper_RedirectsToBecomeDeveloper()
    {
        var context = CreateContext(sessionId: "u1", sessionRole: "DEVELOPER");
        _userServiceMock.Setup(s => s.GetByIdAsync("u1"))
        ...
        context.Result.Should().BeOfType<RedirectToActionResult>();
        context.HttpContext.Session.GetString("Role").Should().Be("CUSTOMER");
    }
}
